import torch
from collections import defaultdict


class Hooker():
    def __init__(self, model, num_lora, desc_emb, loss_fn, l1=False):
        self.num_lora = num_lora
        self.early_exit_proj = model.model.visual.early_exit_proj
        self.desc_emb = desc_emb
        
        self.desc_sim = defaultdict(list)
        self.loss = []
        self.loss_fn = loss_fn
        self.l1 = l1
        
    def clear(self):
        self.loss = []
        self.desc_sim = defaultdict(list)
        
    def compute(self, image_embs):
        image_embs = {k: self.early_exit_proj(v.to("cuda")) for k,v in image_embs.items()}
        image_embs = {k: v / v.norm(dim=-1, keepdim=True) for k,v in image_embs.items()}
        image_embs = {k: [v @ self.desc_emb.t()] for k,v in image_embs.items()}
        
        if self.loss_fn is not None and len(image_embs)>1: 
            self.loss.append(self.loss_fn({k: v for k,v in image_embs.items() if k != 'org'}, self.l1))
        else: 
            for k in image_embs: 
                self.desc_sim[k].append(image_embs[k][0].detach().cpu())
        
    def return_loss(self):
        return torch.stack(self.loss, dim=0).mean()